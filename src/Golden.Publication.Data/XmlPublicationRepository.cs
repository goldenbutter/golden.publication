using System;
using System.Linq; // make sure LINQ extensions are available
...
{
    private readonly List<PublicationModel> _publications;
        ...
        Func<IQueryable<PublicationModel>, IOrderedQueryable<PublicationModel>> orderBy)
    {
        ...
        IReadOnlyList<PublicationModel> result = ordered.ToList();
        return Task.FromResult(result);
    }

    public Task<IReadOnlyList<PublicationModel>> GetByTypeAsync(string publicationType)
    {
        ...
        return Task.FromResult((IReadOnlyList<PublicationModel>)result);
    }

    public Task<IReadOnlyList<PublicationModel>> GetByIsbnAsync(string isbn)
    {
        var result = _publications
